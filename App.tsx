    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/90 backdrop-blur-md border-b border-gray-100 py-4 shadow-sm' : 'bg-transparent py-6'}`}>
      <div className="max-w-[1440px] mx-auto px-10 flex justify-between items-center">
        <div className="flex items-center gap-4 cursor-pointer" onClick={() => onNavigate(AppView.HOME)}>
          <BrandLogo />
          <span className="text-[18px] font-bold text-gray-900 border-l border-gray-200 pl-4 h-6 flex items-center">BFSI Campaign Generator</span>
        </div>
        <div className="flex items-center gap-8">
          <button onClick={() => onNavigate(AppView.CONTACT_US)} className="text-[15px] font-bold text-gray-800 hover:text-[#F97316] transition-colors">Contact Us</button>
          {isAuthenticated ? (
            <div className="flex items-center gap-4">
              <button onClick={() => onNavigate(AppView.DASHBOARD)} className={`px-6 py-2 border rounded-lg font-bold text-[15px] transition-all ${currentView === AppView.DASHBOARD ? 'bg-[#F97316] text-white' : 'bg-white text-gray-800'}`}>Dashboard</button>
              <div className="relative">
                <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="w-10 h-10 rounded-full bg-orange-100 flex items-center justify-center text-[#F97316]"><User size={20} /></button>
                {isProfileOpen && (
                  <div className="absolute right-0 mt-3 w-56 bg-white rounded-xl shadow-xl border p-2">
                    <button onClick={() => { onManageAccount(); setIsProfileOpen(false); }} className="w-full flex items-center gap-3 p-3 text-sm font-bold text-gray-600 hover:bg-gray-50 rounded-lg"><Settings size={16} /> Settings</button>
                    <button onClick={onLogout} className="w-full flex items-center gap-3 p-3 text-sm font-bold text-red-500 hover:bg-red-50 rounded-lg"><LogOut size={16} /> Log Out</button>
                  </div>
                )}
              </div>
            </div>
          ) : (
            <button onClick={onLoginClick} className="px-6 py-2 border border-gray-200 rounded-lg font-bold text-[15px] text-gray-800 hover:bg-gray-50 transition-all">Sign In</button>
          )}
        </div>
      </div>
    </nav>
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="animate-in fade-in duration-700">
          <section className="relative pt-44 pb-24 overflow-hidden bg-white">
            <div className="max-w-[1440px] mx-auto px-10 text-center relative z-10">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gray-100 border border-gray-200 mb-8">
                ...
                <span className="text-[12px] font-bold text-gray-800">Compliant Strategic Engine</span>
              </div>
              <h1 className="text-[72px] font-bold text-[#F97316] leading-[1.1] tracking-tight mb-8">Personalized Campaigns <br/> In Record Time</h1>
              <p className="text-[20px] text-[#475569] max-w-3xl mx-auto mb-12 font-medium">Generate compliant marketing messages for your BFSI customers using AI. Upload your matrix, customize your tone, and deploy.</p>
              <div className="flex flex-col sm:flex-row items-center justify-center gap-5 mb-24">
                <button onClick={handleCreateCampaignRequest} className="px-8 py-4 bg-[#F97316] text-white rounded-lg font-bold text-[18px] hover:bg-[#EA580C] shadow-lg flex items-center gap-3"><Sparkles size={20} /> Create Campaign</button>
                <button onClick={() => {setIsDemoActive(true); setDemoStep(DemoStep.INTRO);}} className="px-8 py-4 bg-white border border-gray-200 text-[#0F172A] rounded-lg font-bold text-[18px] flex items-center gap-3"><BarChart3 size={20} /> View Demo</button>
              </div>
          ...
          <section id="features" className="py-24 bg-white">
            <div className="max-w-[1440px] mx-auto px-10">
              <div className="text-center mb-20"><h2 className="text-[48px] font-bold text-[#0F172A] mb-4">Strategic <span className="text-[#F97316]">Capabilities</span></h2></div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {FEATURES.map((feature, idx) => (
                  <div key={idx} className="bg-white p-8 rounded-xl border border-gray-100 shadow-sm flex flex-col items-start">
                    <div className="w-12 h-12 bg-[#FFF7ED] rounded-lg flex items-center justify-center mb-6">{feature.icon}</div>
                    <h3 className="text-[22px] font-bold text-[#0F172A] mb-3">{feature.title}</h3>
          ...
          <section className="py-24 bg-white">
            <div className="max-w-[1100px] mx-auto px-10">
              <div className="bg-[#F97316] rounded-2xl p-20 text-center flex flex-col items-center shadow-xl">
                <h2 className="text-[44px] font-bold text-[#0F172A] mb-4">Scale Your Compliance Engine</h2>
                <p className="text-[18px] text-white font-medium mb-12">Automate the intersection of finance and creative copy.</p>
                <button onClick={handleCreateCampaignRequest} className="bg-white text-[#F97316] px-10 py-4 rounded-lg font-bold text-[18px] shadow-lg">Get Started Now</button>
              </div>
            </div>
          </section>
        </div>
      ...
      <footer className="bg-[#0F172A] pt-32 pb-16 mt-20">
        <div className="max-w-[1440px] mx-auto px-10 flex flex-col lg:flex-row justify-between items-center gap-8 border-t border-white/5 pt-16">
          <BrandLogo className="brightness-0 invert opacity-50" />
